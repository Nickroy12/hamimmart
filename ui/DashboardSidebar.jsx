...
export default function DashboardSidebar() {
    ...
    <div className="flex flex-col h-full w-full text-foreground bg-white dark:bg-zinc-950">
      ...
      <div className="p-5 border-b border-zinc-100 dark:border-zinc-900 flex flex-col gap-4">
        <div className="flex items-center gap-3">
          {/* Plain CSS Avatar using the user's initial */}
          <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0 bg-orange-100 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 border border-orange-200/50 dark:border-orange-900/30">
        ...
        <div>
          <span className="inline-block px-3 py-1 text-xs font-medium bg-green-50 text-green-600 dark:bg-green-950/30 dark:text-green-400 rounded-full border border-green-200/50 dark:border-green-900/30">
            {session?.user?.role === "admin" 
              ? "Admin" 
              : session?.user?.plan === "chef_free" 
              ? 'Consumer' 
              : session?.user?.plan === "chef_Pro" 
              ? 'Pro Consumer' 
              : 'Premium Consumer'}
          </span>
        </div>
  ...
  return (
    <>
      {/* Mobile Header */}
      <div className="md:hidden border-b border-zinc-200 dark:border-zinc-800 p-4 bg-background flex items-center justify-between">
